# Derma AI - Lesion / Non-Lesion Binary Classification Training
...
# Portable paths: override with environment variables when needed.
INPUT_DIR = Path(os.environ.get("DERMA_DATASET_DIR", "/kaggle/input/datasets"))
OUTPUT_DIR = Path(os.environ.get("DERMA_OUTPUT_DIR", "/kaggle/working"))
...
positive_paths = sorted(set(sum((list_images(root) for root in positive_roots), [])))
negative_paths = sorted(set(sum((list_images(root) for root in negative_roots), [])))
...
negative_paths = negative_paths[: min(len(negative_paths), len(positive_paths) * NEGATIVE_RATIO)]

df = pd.DataFrame({
    "path": positive_paths + negative_paths,
    "label": [1] * len(positive_paths) + [0] * len(negative_paths),
})
...
class RandomLesionLowQualityAugment:
    ...
    def __call__(self, image):
        ...
        if random.random() < 0.65:
            image = image.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.4, 2.2)))
        if random.random() < 0.45:
            buffer = io.BytesIO()
            image.save(buffer, format="JPEG", quality=random.randint(25, 70))
            buffer.seek(0)
        ...
        if random.random() < 0.75:
            image = transforms.ColorJitter(brightness=0.35, contrast=0.35, saturation=0.20, hue=0.04)(image)
        return image


class AddGaussianNoise:
    def __init__(self, p=0.20, mean=0.0, std=0.020):
        self.p, self.mean, self.std = p, mean, std

    def __call__(self, tensor):
        if random.random() < self.p:
            tensor = torch.clamp(tensor + torch.randn_like(tensor) * self.std + self.mean, 0.0, 1.0)
        return tensor
...
class LesionBinaryDataset(Dataset):
    def __init__(self, dataframe, base_transform=None, positive_low_quality_aug=None, tensor_transform=None, train_mode=False):
        ...
        self.positive_low_quality_aug = positive_low_quality_aug
    ...
    def __getitem__(self, idx):
        ...
        label_value = int(self.df.loc[idx, "label"])
        try:
            image = Image.open(img_path).convert("RGB")
        except Exception:
            image = Image.new("RGB", (IMG_SIZE, IMG_SIZE), (0, 0, 0))
        if self.train_mode and label_value == 1 and self.positive_low_quality_aug is not None:
            image = self.positive_low_quality_aug(image)
        if self.base_transform:
            image = self.base_transform(image)
        if self.train_mode and self.tensor_transform:
...
train_base_transform = transforms.Compose([
    ...
    transforms.RandomResizedCrop(IMG_SIZE, scale=(0.78, 1.00), ratio=(0.90, 1.10)),
    transforms.ColorJitter(brightness=0.12, contrast=0.12, saturation=0.08, hue=0.02),
    transforms.ToTensor(),
])
train_tensor_transform = transforms.Compose([
    AddGaussianNoise(p=0.20, std=0.020),
...
def train_one_epoch(model, loader, optimizer):
    ...
    for images, labels in tqdm(loader):
        ...
        all_targets.extend(labels.detach().cpu().numpy().ravel())
...
def save_checkpoint(model, path):
    torch.save({
        ...
        "labels": {"0": "lezyon_degil", "1": "lezyon"},
    }, path)
...
_, val_true, val_probs = evaluate(model, val_loader)
best_threshold = 0.5
best_f1 = -1.0
...
with open(LABELS_PATH, "w", encoding="utf-8") as f:
    json.dump({"0": "lezyon_degil", "1": "lezyon"}, f, indent=2, ensure_ascii=False)
